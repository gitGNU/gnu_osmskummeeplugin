...
insert_port_errors(plugin_data_t *plugin_data, osm_epi_pe_event_t *pe_event)
{
	QUERY(plugin_data,
		...
		"link_integrity=%"PRIuLEAST64","
		"buffer_overrun=%"PRIuLEAST64","
...
sql_add_port_errors(plugin_data_t *plugin_data, osm_epi_pe_event_t *pe_event)
{
	...
	if (QUERY(plugin_data,
			"select symbol_err_cnt, link_err_recover,"
			"       link_downed,rcv_err, rcv_rem_phys_err,"
			"       rcv_switch_relay_err, xmit_discards,"
			"       xmit_constraint_err, rcv_constraint_err,"
			"       link_integrity, buffer_overrun,"
			"       vl15_dropped"
